...
- Produces: `AppearancePreference` (`.system`/`.light`/`.dark`, `resolve(systemIsDark:)`), `ArcadeTheme` (`make(_:)`, 색 프로퍼티 10개, `usesGlow`, `usesHalftone`)
...
- Produces: `SyncScheduler(settings:clock:sleep:perform:)`, `requestSync(reason:)`, `enum SyncReason { timer, foreground, manual }`, `SyncScheduler.State` (`consecutiveFailures`, `lastSyncAt`, `shouldSurfaceFailure`)
