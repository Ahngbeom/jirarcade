...
public final class AppModel {
    ...
    public func start() async {
        phase = .launching
        guard let saved = try? credentials.load(), let saved else {
...
`if persistOnSuccess { try? credentials.save(creds) }` 줄을 아래로 교체한다:
...
```swift
        if persistOnSuccess {
            ...
            if let previous, previous?.email != creds.email {
                try? store.reset()
            }
