...
public struct AppSettings: Sendable, Equatable {
    ...
    public func backoffDelay(afterConsecutiveFailures failures: Int) -> Duration {
        guard failures > 0, let last = backoffSteps.last else { return .zero }
...
/// 이 파일에서 소스 디렉터리까지의 상대 경로.
/// Tests/ArcadeAppTests/ → Packages/Jirarcade/ → Sources/
...
@Test func noOrganizationSpecificDataInSources() throws {
    let forbidden = ["flyingdoctor", "atlassian.net/rest"]
    for file in swiftFiles(in: sourcesDirectory()) {
        let text = try String(contentsOf: file, encoding: .utf8)
        for needle in forbidden {
            #expect(!text.lowercased().contains(needle),
                    "\(file.lastPathComponent)에 \(needle)이 있다")
        }
    }
}
```
