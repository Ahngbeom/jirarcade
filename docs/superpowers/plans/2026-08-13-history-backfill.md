...
- [ ] **Step 6: 마법사가 "지금 무엇으로 채점되고 있는지" 보여준다**

Task 16이 재진입 경로를 만들었지만 **사용자는 여전히 무엇을 고쳐야 하는지 알 수 없다.**
다시 연 마법사에서 과거 이력 상태는 행만 뜨고 선택이 비어 있는데, 실제로는 폴백이
추정한 단계로 채점되고 있다. 실물에서 보류 성격의 상태가 `done`으로 추정돼 완료 전이로
채점되고 있었는데, 화면에는 그 사실이 어디에도 없다.

**폴백을 초기 선택으로 채우면 안 된다.** 그러면 사용자가 확인만 눌러도 추정값 전부가
사용자 매핑으로 승격되고, 이후 폴백 갱신이 영영 이기지 못한다. 대신 행마다 **현재 추정을
표시**한다:

```swift
                    if let guess = model.currentFallbacks.stage(for: entry.name),
                       selection[entry.name] == nil {
                        Text("지금은 '\(guess.label)'로 추정해 채점 중입니다")
                            .font(.caption2)
                            .foregroundStyle(theme.inkTertiary)
                    }
```

`AppModel`에 저장된 폴백을 읽는 값을 노출한다(`WorkflowStore.loadFallbacks()`).
`Stage`에 사용자에게 보여줄 한국어 이름이 없으면 뷰 안에서 매핑한다 — 도메인 타입에
표시 문자열을 넣지 않는다.

설정 화면의 "과거 이력에서 찾은 상태 N개가 추정값으로 채점되고 있습니다"도 개수만
알려주므로, 이 표시가 붙어야 사용자가 실제로 골라낼 수 있다.

- [ ] **Step 7: 매핑을 고치면 화면에 반영된다**

`confirmMapping`이 저장 후 `.ready`로 보내지만 **재집계를 트리거하지 않는다.**
사용자가 잘못된 폴백을 고쳐도 XP·레벨이 그대로여서 아무 일도 안 일어난 것처럼 보인다.
매핑은 채점의 입력이므로 확정 직후 파생 상태를 다시 계산해야 한다.

> 이 경로는 `.ready → .mappingWorkflow → .ready` 전이를 새로 만들었다.
> `RootContent.onChange(of: model.phase)`가 `.ready` 진입 시 동기화 타이머를 재생성하고
> 즉시 동기화를 한 번 보내므로, 매핑을 고칠 때마다 연속 실패 백오프가 리셋된다.
> 매핑 변경 직후의 동기화 자체는 합리적이니 그대로 두되, 재집계가 동기화 완료를
> 기다리지 않고 저장된 이벤트로 바로 돌아야 한다(그래야 즉시 반영된다).
