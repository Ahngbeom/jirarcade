...
| 실패 | 처리 |
|---|---|
| `transitionRejected` (400) | 카드 원위치 + Jira가 준 사유 표시. 필수 필드 때문인 경우가 대부분이라 사유가 실질적 안내가 된다 |
| `unauthorized` (401) | `AppModel`의 기존 만료 경로로 넘긴다. 카드 원위치 |
| `offline` | 카드 원위치 + "연결되지 않았습니다". 재시도는 사용자가 다시 고른다 |
| 그 외 | 카드 원위치 + 종류만 표시 |

실패 사유는 `redactedErrorDescription`을 거친다. Jira 응답 본문에는 이메일이 들어 있고
`transitionRejected(reason:)`가 그것을 그대로 나를 수 있다(v0.1 스펙 §8.4).
...
`issues`·`statusEnteredAt`·`hygiene`은 기존 `refreshDerivedState()` 경로에서 함께 갱신한다.
동기화 성공·백필 완료·재집계가 이미 그 함수를 부르므로 새 갱신 시점을 만들지 않는다.
...
□ 전이가 실패하면 카드가 제자리로 돌아가고 사유가 뜨며 XP가 없다
