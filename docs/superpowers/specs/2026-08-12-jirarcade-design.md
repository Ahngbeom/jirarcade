...
> **데일리 보너스는 "오늘 하루분"만 계산된다 (구현상 한계).** 미러에는 현재 상태만 있어 과거 어느 날의 위생 점수도
> 복원할 수 없다. 따라서 이 보너스는 누적되지 않는다 — 열흘 연속 80점을 넘겨도 +50이지 +500이 아니고,
> 어제 넘겼어도 오늘 못 넘기면 사라진다. 일별로 적립하려면 **일별 위생 스냅샷을 이벤트 로그에 남겨야** 한다
> (`EventKind`에 케이스 추가 — 추가는 안전하다). 계획 2 착수 전에 결정할 것.
