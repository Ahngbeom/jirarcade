...
532개 테스트가 2초 안에 끝납니다. 규칙이 순수 함수이고 네트워크·시계·달력이 전부 주입되기 때문입니다 — 5분 타이머와 30초 쿨다운을 검증하는 테스트도 실제로는 밀리초 안에 돕니다.
...
- **`ArcadeApp`은 SwiftUI를 모릅니다.** 테스트하는 것과 눈으로 확인하는 것의 경계이고, 그래서 532개 테스트가 화면 없이 돕니다
